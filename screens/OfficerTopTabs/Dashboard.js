...
function Dashboard() {
  ...
  const [data, setData] = useState(
    AppCtx.rArticles.filter(
      (article) => +article.published_by === +AppCtx.usermetadata.get_user_id
    )
  );
      ...
      <View style={styles.cardHolder}>
        <DashboardCard
          ...
          number={
            AppCtx.rArticles.filter(
              (article) =>
                +article.published_by === +AppCtx.usermetadata.get_user_id
            ).length
          }
        />
        <DashboardCard
        ...
        {AppCtx.officerposts.length > 0 ? (
          <>
            ...
            <ScrollView style={styles.innerTableHolder}>
              <DataTable2 data={data} onTapHandler={onTapArticleHandler} />
            </ScrollView>
          </>
        ) : (
          <View
